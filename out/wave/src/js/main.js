(() => {
  ...
  $(
    function() {
      var $window = $(window), $oBody = $("body"), $oHeader = $("#header"), $oBasketList = $("#basket_list"), $oToTop = $("#jumptotop"), $oRecommendations = $("#econdaRecommendations"), $oChangeEmail = $("input.oxValidate_enterPass"), $oSearchInput = $("#searchParam"), iHeaderWrapperHeight = parseInt($oHeader.height()), blIsCheckout = $oBody.hasClass("is-checkout"), $oContentWrapper = $("#content");
      ...
      $("#header .dropdown-menu input, #header .dropdown-menu label, #header .dropdown-menu button, #header .dropdown-menu").click(
        function(e) {
          e.stopPropagation();
        }
      );
      ...
      $oHeader.find(".menu-dropdowns button[data-href]").click(
        function(e) {
          var $this = $(this);
          if ($(window).width() <= 767) {
            e.stopPropagation();
      ...
      if ($oRecommendations.length) {
        $.get(
          window.sBaseUrl + "cl=tpl&tpl=ajax_econda_recommendations.tpl&actcl=" + sActCl,
      ...
      if ($oFilterList.length) {
        $oFilterList.find(".dropdown-menu li").click(
          function() {
            var $this = $(this);
      ...
      var $oSidebar = $("#sidebar"), $oCategoryBox = $oSidebar.find(".box.categorytree");
      if ($oCategoryBox.length) {
        $oCategoryBox.find(".toggleTree").on(
          "click touch",
          function() {
            var $this = $(this), $oCategoryTreeBox = $this.parents(".categorytree").find(".categoryBox");
      ...
      if ($oSelectionLists.length) {
        $oSelectionLists.find(".dropdown-menu li").click(
          function(e) {
            e.preventDefault();
            var $this = $(this);
            $this.parent().prev().val($this.children().first().data("selection-id"));
      ...
      Wave.initEvents = function() {
        ...
        $(".hasPopover").popover();
        var $oSelectionLists2 = $("#detailsMain .selectorsBox");
        if ($oSelectionLists2.length) {
          $oSelectionLists2.find(".dropdown-menu li").click(
            function(e) {
              e.preventDefault();
              var $this = $(this), $a = $this.children().first(), $ul = $this.parent();
      ...
      function reRenderMainNav() {
        var $oMainNav = $("#mainnav").find(".navbar-collapse"), $oNavList = $("#navigation"), $oMoreLinks = $oNavList.find(".moreLinks");
        if ($window.width() <= 755) {
        ...
        if ($oMoreLinks.length) {
          ...
          var oMoreLinkElem = document.createElement("li"), oMoreLinkAElem = document.createElement("a"), oMoreLinkUlElem = document.createElement("ul");
        ...
        var iMainNavWidth = $oMainNav.width() - $("#navbarSupportedContent > .fixed-header-actions").width(), $oNavItems = $oNavList.find("> li").not(".moreLinks"), iNavItemsWidth = 0, aMoreLinkElems = [];
      ...
      if ($oBasketList.length) {
        $("#basket_form").on(
          "submit",
          function(e) {
            var $this = $(this), $oHiddenXs = $this.find(".hidden-xs"), $oVisibleXs = $this.find(".visible-xs");
        ...
        if ($window.width() <= 996) {
          $oBasketList.find(".toggle-actions").click(
            function(e) {
              e.preventDefault();
              var $this = $(this), $oToggable = $this.parents("li").first().find(".row.collapse");
      ...
      $(".basketItemDesc .selectorsBox .dropdown-menu li a", $oContentWrapper).click(
        function(e) {
          e.preventDefault();
          var $this = $(this);
      ...
      if ($oChangeEmail.length) {
        var sOldMail = $oChangeEmail.val(), $oPasswordElem = $(".oxValidate_pwd");
        $oChangeEmail.keyup(
          function(e) {
